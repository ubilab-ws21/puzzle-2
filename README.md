...
The power switches are protected by a tap-tap pattern for which the players have to solve a riddle. Once you get the pattern right you get access to the power switches. But, there are lots of switches and you need to turn on few of them. One wrong switch can drain out the emergency backup. The order of the switches which needs to be turned on will be given by the puzzle displayed in the OLED Display. You get the right sequence of switches, the server turns on.
